...
export const analyzeStudent = (student: any) => {
  ...
  if (student.scoreTrend > 0.05) academicTrend = 'Improving';
  else if (student.scoreTrend < -0.05) academicTrend = 'Declining';

  // Attendance Trend
  let attendanceTrend: 'Up' | 'Flat' | 'Down' = 'Flat';
  if (student.engagementTrend > 0.1) attendanceTrend = 'Up';
  else if (student.engagementTrend < -0.1) attendanceTrend = 'Down';

  // Engagement Level
  let engagementLevel: 'Low' | 'Medium' | 'High' = 'Medium';
  if (student.participationRate >= 0.85) engagementLevel = 'High';
  else if (student.participationRate < 0.6) engagementLevel = 'Low';

  // Last Activity
  let lastActivity = 'Unknown';
  if (student.lastSubmissionDaysAgo === 0) lastActivity = 'Today';
  else if (student.lastSubmissionDaysAgo === 1) lastActivity = 'Yesterday';
  else if (student.lastSubmissionDaysAgo < 7) lastActivity = `${student.lastSubmissionDaysAgo} days ago`;
  else lastActivity = `${Math.floor(student.lastSubmissionDaysAgo / 7)} week ago`;
  ...
  else if (riskScore >= 2) riskLevel = 'Needs Attention';

  // Recent Change Insight
  let recentChangeInsight = "Maintaining consistent performance levels.";
  if (student.scoreTrend > 0.1) recentChangeInsight = "Academic performance is improving despite low participation.";
  else if (student.attendanceRate < 0.8 && student.attendanceRate < (student.historicalBaselineScore / 100)) recentChangeInsight = "Attendance has declined over the past two weeks.";
  else if (student.lastSubmissionDaysAgo > 3) recentChangeInsight = "Recent submission gaps detected after consistent history.";
  else if (student.engagementTrend > 0.2) recentChangeInsight = "Significant increase in classroom engagement recently.";
  ...
  if (riskLevel === 'Stable') {
    const factors = [];
    if (student.attendanceRate > 0.9) factors.push("<b>consistent attendance</b>");
    if (academicTrend === 'Improving' || academicTrend === 'Stable') factors.push("<b>steady academic scores</b>");
    if (engagementLevel === 'High' || engagementLevel === 'Medium') factors.push("<b>active engagement</b>");
  ...
  if (hasScore) {
    if (score > 85) topFactors.push('Strong weighted academic performance based on historical and recent results');
    else if (score < 50) topFactors.push('Academic challenges detected in combined performance metrics');
  } else {
    topFactors.push('Insufficient academic data for historical/exam weighted score');
  }

  if (student.submissionConsistency > 0.9) topFactors.push('Exceptional reliability in work submission');
  if (student.participationRate > 0.85) topFactors.push('Highly active in class discussions');
  if (student.scoreTrend > 0.1) topFactors.push('Strong upward academic trajectory');
  if (student.attendanceRate > 0.95) topFactors.push('Perfect or near-perfect attendance');
  
  if (topFactors.length === 0) topFactors.push('General adherence to classroom norms');
...
export const processStudentUpload = async (students: any[]) => {
...
export const getClassSummary = async () => {
  ...
  if (avgAttendance < 0.9) keyTrends.push('Overall class attendance is slightly below historical norms.');
  ...
  try {
    ...
    if (provider) {
      ...
      if (summaryRes.text) aiSummary = summaryRes.text;
      if (changesRes.text) recentChangesSummary = changesRes.text;
      if (planRes.text) {
